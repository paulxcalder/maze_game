...
static char login_len = 0;
static char login_error = 0; //0 - все хорошо   1 все плохо
...
static char gen_ok = 0; //1 все хорошо 0 - плохо
static int show_solution = 0;

//в игре
static long long current_seed  = 0;
static float play_elapsed  = 0.0f; //время игры
static char timer_running = 0; //запуск таймера
static char score_saved = 0; //сохранен ли прогресс

//экран ввода цифр
static char key_seed_buf[22] = {0};
static char key_seed_len = 0;
static char key_inputW = 0;
static char key_inputH = 0;
static char key_active = -1;   //0 - ширина   1 - высота  2 - seed
static char key_errorFlag = 0;
...
{
    char hot = CheckCollisionPointRec(m, backBtn);
...
static void DrawMenu(Font font, Screen *screen)
{
    ...
    //играть
    char hotStart = CheckCollisionPointRec(m, btnStart);
    ...
    //помощь
    char hotHelp = CheckCollisionPointRec(m, btnHelp);
    ...
    //справка
    char hotAbout = CheckCollisionPointRec(m, btnAbout);
    ...
    //лидеры
    char hotLead = CheckCollisionPointRec(m, btnLead);
    ...
    //выход
    char hotExit = CheckCollisionPointRec(m, btnExit);
...
static void DrawLogin(Font font, Screen *screen)
{
    ...
    static unsigned int blink_login = 0;
    blink_login++;
    char displayBuf[36];
    if (((blink_login / 36) & 1) == 0)
    ...
    char hotConf = CheckCollisionPointRec(m, confirmBtn);
    ...
    char doConfirm = (hotConf && IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) || IsKeyPressed(KEY_ENTER);
    if (doConfirm) {
        if (login_len > 0) {
            char logbuf[80];
    ...
    if (login_error) {
        ...
        DrawTextEx(font, "Логин не может быть пустым!",
                   (Vector2){(1920 - errt.x) * 0.5f, confirmBtn.y + confirmBtn.height + 18},22, 1, RED);
    }
...
static void DrawModeSelect(Font font, Screen *screen)
{
    ...
    //solo
    char hotSolo = CheckCollisionPointRec(m, btnSolo);
                     ...
                     int *active, int *px, int *py, int *errorFlag)
{
    ...
    char hotGen = CheckCollisionPointRec(m, genBtn);
    ...
    DrawTextEx(font, "СГЕНЕРИРОВАТЬ",
               (Vector2){genBtn.x + (genBtn.width  - gt.x) * 0.5f,
                         genBtn.y + (genBtn.height - gt.y) * 0.5f},24, 1, WHITE);
...
{
    gen_timer += GetFrameTime(); //60frames  -  0.0167 сек
...
static void DrawPlaying(Font font, Screen *screen, int *px, int *py)
{
    ...
    Vector2 m = GetMousePosition();
    char hotMenu = CheckCollisionPointRec(m, menuBtn);
    ...
    //автопроход
    Rectangle autoBtn = {1920.0f - 300.0f, 900.0f, 280.0f, 42.0f};
    char hotAuto = CheckCollisionPointRec(m, autoBtn);
    ...
    DrawTextEx(font, autoLabel,
               (Vector2){autoBtn.x + (autoBtn.width  - alt.x) * 0.5f,autoBtn.y + (autoBtn.height - alt.y) * 0.5f},
               20, 1, WHITE);
    if (hotAuto && IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
        show_solution = !show_solution;
        if (show_solution)
            find_way(map, 1, 1, 1);   //отметить
        else
            find_way(map,1,1,0); //снять метки
    }
...
static void DrawKeyInput(Font font, Screen *screen, int *px, int *py)
{
    ...
    blink_key++;
    char seedDisplay[24];
    if (key_active == 2 && ((blink_key / 36) & 1) == 0)
...
{
    int malloc_count, realloc_count, calloc_count, free_count;
    ...
    while (!WindowShouldClose()) {
        ...
        switch (screen) {
            ...
            case KEY_INPUT:  
            DrawKeyInput(font, &screen, &px, &py);
            break;
        }
    ...
    free_map();
    UnloadFont(font);
    FILE* f_mem = fopen("memstat.txt", "w");
    if(f_mem != NULL){
        get_memstat(&malloc_count, &realloc_count, &calloc_count, &free_count);
        fprintf(f_mem, "malloc: %d\nrealloc: %d\ncalloc: %d\nfree: %d\n",
            malloc_count, realloc_count, calloc_count, free_count);
            fclose(f_mem);
    }
    CloseWindow();
}